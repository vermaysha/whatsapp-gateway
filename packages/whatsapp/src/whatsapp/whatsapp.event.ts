...
export async function listenWhatsappEvent(sock: WASocket, deviceId: string) {
  // Contact events
  sock.ev.on('contacts.upsert', (contacts) =>
    contactEvent(contacts, sock, deviceId),
  )
  sock.ev.on('contacts.update', (contacts) =>
    contactEvent(contacts, sock, deviceId),
  )

  sock.ev.on('chats.upsert', (args) => chatEvent(args, sock, deviceId))
  sock.ev.on('chats.update', (args) => chatEvent(args, sock, deviceId))
  ...
  sock.ev.on('messages.upsert', (args) =>
    messageUpsertEvent(args, sock, deviceId),
  )
